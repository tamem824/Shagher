<x-app-layout>
    <div class="max-w-4xl mx-auto p-8">
        ...
        <form action="{{ route('admin.jobs.update', $job->id) }}" method="POST" class="mt-8 space-y-6 bg-white dark:bg-gray-800 shadow-lg rounded-2xl p-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $job->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $job->expired_at) }}"
                           class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    ...
                    <input type="date" id="expiration_date" name="expiration_date" value="{{ old('expiration_date', $job->expiration_date) }}"
                           class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div>
                ...
                <select id="career_level_id" name="career_level_id"
                    ...
                    @foreach($tag_careers as $tag)
                        <option value="{{ $tag->id }}" {{ old('career_level', $job->career_level) == $tag->id ? 'selected' : '' }}>
                            {{ $tag->name }}
                        </option>
                    @endforeach
                </select>
                @error('career_level') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
            </div>
            {{-- Employment type --}}
            <div>
            ...
            <div>
                ...
                <input type="text" id="job_type" name="job_type" value="{{ old('job_type', $job->job_type) }}"
                       class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            ...
            @foreach($textareas as $name)
                <div>
                    ...
                    <textarea id="{{ $name }}" name="{{ $name }}[]" rows="3"
                              class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old("$name.0", $job->$name) }}</textarea>
            ...
            <div>
                ...
                <select id="tag_id" name="tag_id[]" multiple
                        class="w-full mt-1 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    @foreach($tags as $tag)
                        <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tag_id', $job->tags->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $tag->name }}</option>
                    @endforeach
                </select>
